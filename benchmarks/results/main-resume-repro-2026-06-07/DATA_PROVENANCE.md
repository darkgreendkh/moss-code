...
| `moss-benchmark-core-report.md` | 自动生成的核心 benchmark 汇总 |
...
from moss.evaluation.evaluator import run_harness_regression_v2
...
run_harness_regression_v2(
    ...
    workspace_root=Path("/tmp/moss-main-resume-workspaces"),
)
...
write_benchmark_core_report(
    report_path=out / "moss-benchmark-core-report.md",
...
| 7 类工具 | `moss/tools.py` 里 6 个基础工具，加上 `delegate`，总共 7 个可暴露工具。 | `BASE_TOOL_SPECS` 有 6 个：`list_files/read_file/search/run_shell/write_file/patch_file`，`legal_tool_names()` 额外加入 `delegate` |
| 3 类运行工件 | 每次 run 固定落盘 `task_state.json`、`trace.jsonl`、`report.json`。 | `moss/run_store.py` 的 `task_state_path()`、`trace_path()`、`report_path()` |
...
| 汇总报告 | `moss-benchmark-core-report.md` |
