...
连这份文档里的示例也会命中检测器自己。把误报变成"任务直接失败"比漏报还难受。
而"接下来都要问一次"这个后果，对真攻击有效，对误报只是多按几次 y。

因为误报常见，注入触发的审批提示要**给足判断依据、且是一次性的**：

- 提示里连同命中的模式名一起打出**命中原文**和**来源**（`read_file:docs/…`）。只给
  模式名的话，用户没法判断这是"读到了自己文档里的示例"这种误报，还是真有外部
  文本在指挥 agent——两者的正确处置完全相反。
- 注入嫌疑期间的审批**不读也不写审批记忆**，选项只有 `y = once · N = no`。否则用户
  看着"疑似注入"按下的 never 会把整类命令在本会话里永久禁掉，而之前记过的 always
  又会让可疑动作直接跳过审批（`execution/service.py::_ask_for_approval`）。
