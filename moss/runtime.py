...
class Moss:
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Moss.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Moss.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Moss.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Moss.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Moss.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Moss.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Moss.retry_notice()
        if "<final>" in raw:
            final = Moss.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Moss.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Moss.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Moss.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Moss.extract_raw(body, key)
