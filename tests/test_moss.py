...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".moss").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".moss" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "moss" / "agent" / "welcome" / "screen"
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = moss_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = moss_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    assert report["prompt_version"] == "p1"
    assert report["run_manifest"]["prompt_version"] == "p1"
    assert report["prompt_metadata"]["prompt_version"] == "p1"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .moss/.\n"
    ...
    index_path = tmp_path / ".moss" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".moss" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".moss" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .moss/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".moss" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".moss" / "memory" / "topics" / "dependency-facts.md"
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(moss_pkg.main)
    assert callable(moss_pkg.build_agent)
    assert callable(moss_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "moss", "--help"],
        capture_output=True,
        text=True,
    )
